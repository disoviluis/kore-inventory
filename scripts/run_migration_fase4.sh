#!/bin/bash
# Script para ejecutar migración ignorando errores de columnas duplicadas
...
# Ejecutar cada ALTER TABLE individualmente, ignorando errores 1060 (columna duplicada)
mysql -h $DB_HOST -u $DB_USER -p"$DB_PASS" $DB_NAME << 'EOF' 2>&1 | grep -v "Duplicate column"
